"""
train_ddp.py - Distributed Data Parallel (DDP) version of EQUICAT training

This script implements distributed training for the EQUICAT model using PyTorch's
DistributedDataParallel (DDP). It allows for efficient multi-GPU training to handle
larger datasets and speed up the training process.

Author: Utkarsh Sharma (Modified for DDP by Claude)
Version: 2.0.0
...
def setup_logging(log_file, rank):
    """
    Set up logging for the DDP training process.
    ...
    file_handler = logging.FileHandler(f"{log_file}_rank{rank}", mode='w')
    ...
    # Add rank information to the logger
...
def calculate_avg_num_neighbors_and_unique_atomic_numbers(dataset, device, rank):
    """
    Calculate the average number of neighbors and unique atomic numbers in the dataset.
    
    Args:
        dataset (MultiFamilyConformerDataset): The dataset to analyze.
    ...
    all_total_neighbors = [torch.tensor(0, device=device) for _ in range(dist.get_world_size())]
    all_total_atoms = [torch.tensor(0, device=device) for _ in range(dist.get_world_size())]
    dist.all_gather(all_total_neighbors, torch.tensor(total_neighbors, device=device))
    dist.all_gather(all_total_atoms, torch.tensor(total_atoms, device=device))
    
    total_neighbors = sum(all_total_neighbors).item()
    total_atoms = sum(all_total_atoms).item()
    
    avg_neighbors = total_neighbors / total_atoms if total_atoms > 0 else 0
    
    # Gather unique atomic numbers from all processes
    all_unique_atomic_numbers = [None for _ in range(dist.get_world_size())]
    dist.all_gather_object(all_unique_atomic_numbers, list(unique_atomic_numbers.keys()))
    
    # Combine unique atomic numbers from all processes
    combined_unique_atomic_numbers = set()
    for nums in all_unique_atomic_numbers:
        combined_unique_atomic_numbers.update(nums)
    
    logger.info(f"Calculation complete. Average neighbors: {avg_neighbors}")
    return avg_neighbors, sorted(list(combined_unique_atomic_numbers))
...
def process_sample(model, sample, device, embedding_type):
    ...
    Args:
        model (nn.Module): The EQUICAT model.
        sample (list): List of molecule data.
        device (torch.device): The device to use for computations.
        embedding_type (str): Type of embedding combination to use.
    
    Returns:
        list: List of tuples containing combined embeddings, keys, and families.
    """
    sample_embeddings = []
    for atomic_data_list, key, family in sample:
        ...
        averaged_embeddings = process_molecule_conformers(molecule_embeddings, model.module.non_linear_readout.irreps_out)
...
def compute_contrastive_loss(sample_embeddings, temperature=0.1):
    ...
    Args:
        sample_embeddings (list): List of tuples containing embeddings, keys, and families.
...
def train_equicat(model, dataset, device, embedding_type, scheduler_type, args, start_epoch, rank, world_size):
    """
    Train the EQUICAT model using Distributed Data Parallel (DDP).
    
    Args:
        model (nn.Module): The EQUICAT model wrapped in DDP.
        dataset (MultiFamilyConformerDataset): The dataset to train on.
    ...
    steps_per_epoch = num_samples // world_size
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        epoch_loss = 0.0
        dataset.reset()
        sampler.set_epoch(epoch)  # Set the epoch in the sampler

        epoch_embeddings = {}

        for sample_idx, sample in enumerate(dataset):
            logger.info(f"Processing sample {sample_idx+1}/{steps_per_epoch}")
            optimizer.zero_grad()
            ...
            logger.info(f"Epoch [{epoch+1}/{EPOCHS}], Sample [{sample_idx+1}/{steps_per_epoch}], "
        ...
        all_epoch_embeddings = [None for _ in range(world_size)]
        dist.all_gather_object(all_epoch_embeddings, epoch_embeddings)
        
        # Combine embeddings from all processes
        if rank == 0:
            combined_epoch_embeddings = {}
            for proc_embeddings in all_epoch_embeddings:
                for key, embeddings in proc_embeddings.items():
                    if key not in combined_epoch_embeddings:
                        combined_epoch_embeddings[key] = []
                    combined_epoch_embeddings[key].extend(embeddings)
            
            for key, embeddings in combined_epoch_embeddings.items():
        ...
        if avg_epoch_loss < best_loss:
            ...
            best_model = model.module.state_dict()  # Save the model without DDP wrapper
            patience_counter = 0
            is_best = True
        ...
        if rank == 0 and ((epoch + 1) % CHECKPOINT_INTERVAL == 0 or is_best):
            save_checkpoint(epoch, model.module, optimizer, scheduler, avg_epoch_loss, OUTPUT_PATH, is_best)
    ...
    if rank == 0:
        ...
        model.module.load_state_dict(torch.load(best_model_path)['model_state_dict'])
        logger.info("Best model loaded successfully")

    # Synchronize all processes after loading the best model
    dist.barrier()
    ...
    with torch.no_grad():
        for family, keys in dataset.family_keys.items():
            ...
            for key in keys:
                ...
                averaged_embedding = process_molecule_conformers(molecule_embeddings, model.module.non_linear_readout.irreps_out)
    ...
    # Gather final embeddings from all processes
    all_final_embeddings = [None for _ in range(world_size)]
    dist.all_gather_object(all_final_embeddings, final_molecule_embeddings)

    # Combine final embeddings from all processes
    if rank == 0:
        combined_final_embeddings = {}
        for proc_embeddings in all_final_embeddings:
            combined_final_embeddings.update(proc_embeddings)
        
        logger.info(f"Computed final embeddings for {len(combined_final_embeddings)} molecules")
        logger.info(f"Total conformers processed: {total_conformers}")

        # Save the final molecule embeddings
        save_final_embeddings(combined_final_embeddings, OUTPUT_PATH)
...
def main(rank, world_size, args):
    """
    Main function to set up and run the distributed training process.
    ...
    setup_logging(f"{OUTPUT_PATH}/training", rank)
    logger.info("Starting EQUICAT distributed training")
    ...
    avg_num_neighbors, unique_atomic_numbers = calculate_avg_num_neighbors_and_unique_atomic_numbers(dataset, device, rank)
    if rank == 0:
    ...
    trained_model = train_equicat(model, dataset, device, args.embedding_type, args.scheduler, args, start_epoch, rank, world_size)
    logger.info("Training process completed")

    cleanup()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EQUICAT Model Distributed Training")
