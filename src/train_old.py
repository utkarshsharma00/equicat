...
# NUM_ENSEMBLES = 4  # Number of ensemble models to be trained and averaged.
...
#                 processed_embeddings = process_conformer_ensemble(embeddings, model.non_linear_readout.irreps_out)
...
#                 combined = vector.view(vector.shape[0], -1)
#             else:
#                 continue  # Skip this method if both scalar and vector are None
...
#             combined = vector.view(vector.shape[0], -1)
#         else:
#             return torch.tensor(0.0, device=scalar.device if scalar is not None else vector.device, requires_grad=True)
...
#             scalars = [scalar for scalar, _ in batches if scalar is not None]
#             vectors = [vector for _, vector in batches if vector is not None]
            
#             if scalars:
#                 all_scalars = torch.cat(scalars, dim=0)
...
#                 scalar, vector = embeddings[method]
#                 if scalar is not None:
...
#             if normalize:
#                 combined = F.normalize(combined, p=2, dim=-1)
#             pairwise_distances = torch.cdist(combined.unsqueeze(0), combined.unsqueeze(0))
...
#                 combined.append(vector.view(-1))
#         if not combined:
#             return torch.tensor(0.0, device=next(iter(avg_embeddings.values()))[embedding_type][0].device, requires_grad=True)
#         combined = torch.cat(combined)
#         if normalize:
#             combined = F.normalize(combined, p=2, dim=-1)
#         pairwise_distances = torch.cdist(combined.unsqueeze(0), combined.unsqueeze(0))
...
#     logging.info(f"Temporary dataset created with {len(temp_dataset.keys)} ensembles")
